...
class MemoryStore implements ManagementStore{
  user!:AuthUser&{passwordHash:string}; sessions=new Map<string,AuthUser>(); candidates:AgentCandidate[]=[];records:any[]=[];runs:any[]=[];
  ...
  async queueAgentRun(agent_key:"chat_archive"|"todo_reminder"|"owner_alert",_actor:string){const run={id:randomUUID(),agent_key,status:"queued",requested_at:new Date().toISOString()};this.runs.push(run);return run} async listAgentRuns(){return this.runs}
}
